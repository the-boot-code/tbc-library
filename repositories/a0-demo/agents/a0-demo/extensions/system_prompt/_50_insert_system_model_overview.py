...
def get_prompt_file(agent: Agent):
    return "/a0/prompts/system/model_overview/model_overview.md"

def read_prompt(agent: Agent):
    prompt_file = get_prompt_file(agent)
    if files.exists(prompt_file):
        # Pass agent config and info as kwargs so they're available to model_overview.md template
