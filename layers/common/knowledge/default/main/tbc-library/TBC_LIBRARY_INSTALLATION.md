...
- Review the cloned settings file for the new agent (for example `layers/a0-myagent/tmp/settings.json` on the host, visible inside the container as `/a0/tmp/settings.json`). Fields such as `agent_profile`, `agent_knowledge_subdir`, and `agent_memory_subdir` will either reflect the source container or any values you passed via `knowledge_dir` or `memory_subdir` to `create_agent.sh`; adjust them if you want the destination to use a different profile, knowledge subdirectory, or memory subdirectory. After you have chosen an `agent_memory_subdir`, you can confirm that the **live** behaviour file for that agent is present at `/a0/memory/<agent_memory_subdir>/behaviour.md` inside the container (with the host mirror at `layers/a0-myagent/memory/<agent_memory_subdir>/behaviour.md`), and treat other `behaviour.md` files you may see elsewhere as backups or historical copies unless this deployment's documentation clearly states otherwise. Using a broad `find / -name behaviour.md` scan is usually unnecessary and can be confusing when many backups exist; prefer the configuration-driven method based on `agent_memory_subdir`.
