...
Within `/a0/memory`, subdirectories such as `/a0/memory/<agent_memory_subdir>` map back to `layers/${CONTAINER_NAME}/memory/<agent_memory_subdir>` on the host. The `agent_memory_subdir` field in `/a0/tmp/settings.json` (which `create_agent.sh` can set via its optional `memory_subdir` argument) selects which memory subdirectory is active for this agent. This is the same directory where `_20_behaviour_prompt.py` looks for the live `behaviour.md` file (`/a0/memory/<agent_memory_subdir>/behaviour.md`), so your behaviour rules are part of the same memory root as your other runtime data. This makes the memory root both self-revealing (discoverable from inside the container) and backup-friendly: you can safely snapshot or copy the corresponding host directory to preserve conversational state and the active `behaviour.md`, while still relying on tools and the Web UI for structured edits.
