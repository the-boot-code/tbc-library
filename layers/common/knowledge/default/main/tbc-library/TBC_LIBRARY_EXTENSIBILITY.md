...
- Core engine extensions:
  ...
  - `_20_behaviour_prompt.py` injects behaviour rules by reading a memory-backed `behaviour.md` file if present, or a default, and passing it as `rules` into `agent.system.behaviour.md`. By default it looks for this file under `/a0/memory/<agent_memory_subdir>/behaviour.md`, where `agent_memory_subdir` is read from `/a0/tmp/settings.json` (host mirror `layers/${CONTAINER_NAME}/memory/<agent_memory_subdir>/behaviour.md`). Agents should not edit this file directly; instead they use the `behaviour_adjustment` tool (`agent.system.tool.behaviour.md`), which updates the memory-backed behaviour file through the normal tool pipeline. When a human or orchestrator clones agents with `create_agent.sh` and passes `memory_subdir`, each destination agent can have its own `/a0/memory/<memory_subdir>/behaviour.md` for isolated behaviour and long-term memory.
