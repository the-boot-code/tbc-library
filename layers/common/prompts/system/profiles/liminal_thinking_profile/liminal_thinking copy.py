...
class ProfileLoader:
    ...
    def _initialize_system_control(self) -> bool:
        ...
        try:
            ...
            if not self.system.is_feature_enabled("liminal_thinking_profile_control"):
                disabled_msg = f"⚠️ Liminal Thinking Control is currently disabled (Security profile: {self.system.get_active_profile()})"
                PrintStyle().hint(disabled_msg)
                self.profile_content = f"# Liminal Thinking Profile\n\n{disabled_msg}\n\nPlease contact your system administrator to enable this feature."
                return False
            return True
    ...
    def _load_profile_data(self) -> None:
        ...
        try:
            self.active_profile = self.system.get_active_liminal_thinking_profile()
            liminal_state = self.system.get_liminal_thinking_state()
            self.features = liminal_state.get('features', {})
            self.features_list = [f for f, cfg in self.features.items() if cfg.get('enabled')]
            PrintStyle().info(f"✓ Loaded liminal thinking profile: {self.active_profile}")
    ...
    def _load_profile_content(self, **kwargs) -> None:
        ...
        profile_path = f"/a0/prompts/system/profiles/liminal_thinking/profiles/{self.active_profile}.md"
        ...
        try:
            ...
            if self.active_profile != "default":
                try:
                    self.active_profile = "default"
                    default_path = "/a0/prompts/system/profiles/liminal_thinking/profiles/default.md"
    ...
    def build_response(self) -> Dict[str, str]:
        ...
        if not self.features_list:
            # status = f"⚠️ {'DEFAULT' if self.active_profile == 'default' else self.active_profile.upper()} (no active features)"
        ...
        return {
            "liminal_thinking_profile": self.profile_content,
            "liminal_thinking_features": features_display,
            "liminal_status": status
        }


class LiminalThinkingProfile(VariablesPlugin):
    """Handles loading and managing the Liminal Thinking profile and its features."""
    ...
    def get_variables(self, file: str, backup_dirs: Optional[List[str]] = None, **kwargs) -> Dict[str, str]:
        ...
        if not loader._initialize_system_control():
            return {
                "liminal_thinking_profile": loader.profile_content,
                "liminal_thinking_features": "(feature disabled)",
