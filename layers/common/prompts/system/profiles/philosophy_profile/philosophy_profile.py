...
class ProfileLoader:
    ...
    def __init__(self):
        ...
        self.module_path = "prompts/system/profiles/philosophy_profile"
    ...
    def _load_profile_data(self) -> None:
        ...
        try:
            self.active_profile = self.system.get_active_profile("philosophy")
            philosophy_state = self.system.get_state("philosophy")
            self.features = philosophy_state.get('features', {})
            self.features_list = [f for f, cfg in self.features.items() if cfg.get('enabled')]
            PrintStyle().info(f"✓ Loaded philosophy profile: {self.active_profile} (from system)") if self.profile_info else None
