...
class ProfileLoader:
    ...
    def _initialize_system_control(self) -> bool:
        ...
        try:
            ...
            if not self.system.is_feature_enabled("reasoning_profile_control"):
                disabled_msg = f"⚠️ External Reasoning Control is currently disabled (Security profile: {self.system.get_active_profile()})"
                PrintStyle().hint(disabled_msg)
                self.profile_content = f"# External Reasoning Profile\n\n{disabled_msg}\n\nPlease contact your system administrator to enable this feature."
                return False
            return True
    ...
    def _load_profile_data(self) -> None:
        ...
        try:
            self.active_profile = self.system.get_active_external_reasoning_profile()
            reasoning_state = self.system.get_external_reasoning_state()
            self.features = reasoning_state.get('features', {})
            self.features_list = [f for f, cfg in self.features.items() if cfg.get('enabled')]
            PrintStyle().info(f"✓ Loaded external reasoning profile: {self.active_profile}")
    ...
    def _load_profile_content(self, **kwargs) -> None:
        ...
        profile_path = f"/a0/prompts/system/profiles/reasoning_profile/external/profiles/{self.active_profile}.md"
        ...
        try:
            ...
            if self.active_profile != "default":
                try:
                    self.active_profile = "default"
                    default_path = "/a0/prompts/system/profiles/reasoning_profile/external/profiles/default.md"
    ...
    def _load_feature_content(self, **kwargs) -> str:
        ...
        for feature in self.features_list:
            try:
                ...
                feature_path = f"/a0/prompts/system/profiles/reasoning_profile/external/features/{ref_file}"
    ...
    def build_response(self) -> Dict[str, str]:
        ...
        return {
            "external_reasoning_profile": self.profile_content,
            "external_reasoning_features": features_display,
            "external_reasoning_status": status
        }


class ExternalReasoningProfile(VariablesPlugin):
    """Handles loading and managing the External Reasoning profile and its features."""
    ...
    def get_variables(self, file: str, backup_dirs: Optional[List[str]] = None, **kwargs) -> Dict[str, str]:
        ...
        if not loader._initialize_system_control():
            return {
                "external_reasoning_profile": loader.profile_content,
                "external_reasoning_features": "(feature disabled)",
                "external_reasoning_status": "⛔ DISABLED"
            }
