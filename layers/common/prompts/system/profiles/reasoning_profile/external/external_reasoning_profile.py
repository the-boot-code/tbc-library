...
class ProfileLoader:
    ...
    def __init__(self):
        ...
        self.module_path = "prompts/system/profiles/reasoning_profile/external"
    ...
    def _load_profile_data(self) -> None:
        ...
        try:
            ...
            reasoning_state = self.system.get_state("reasoning_external")
            self.features = reasoning_state.get('features', {})
            self.features_list = [f for f, cfg in self.features.items() if cfg.get('enabled')]
            PrintStyle().info(f"✓ Loaded external reasoning profile: {self.active_profile} (from system)") if self.profile_info else None
