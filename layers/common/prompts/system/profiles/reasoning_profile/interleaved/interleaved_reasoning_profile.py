...
class ProfileLoader:
    ...
    def _load_profile_data(self) -> None:
        ...
        try:
            self.active_profile = self.system.get_active_interleaved_reasoning_profile()
            reasoning_state = self.system.get_interleaved_reasoning_state()
            self.features = reasoning_state.get('features', {})
            self.features_list = [f for f, cfg in self.features.items() if cfg.get('enabled')]
            PrintStyle().info(f"✓ Loaded interleaved reasoning profile: {self.active_profile}")
    ...
    def _load_profile_content(self, **kwargs) -> None:
        ...
        profile_path = f"/a0/prompts/system/profiles/reasoning_profile/interleaved/profiles/{self.active_profile}.md"
        ...
        try:
            ...
            if self.active_profile != "default":
                try:
                    self.active_profile = "default"
                    default_path = "/a0/prompts/system/profiles/reasoning_profile/interleaved/profiles/default.md"
