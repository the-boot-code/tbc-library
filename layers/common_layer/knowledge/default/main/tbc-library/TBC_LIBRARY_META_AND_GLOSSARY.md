...
- **Memory roots (`agent_memory_subdir`, `/a0/memory`, `behaviour.md`)**  
  - The **Config-First Rule** and step-by-step recipe for locating your memory root and `behaviour.md`: [TBC_LIBRARY_AGENT_REASONING.md → Config-First Rule](TBC_LIBRARY_AGENT_REASONING.md#config-first-rule) and [Memory vantage and backup-safe files](TBC_LIBRARY_AGENT_REASONING.md#memory-vantage-and-backup-safe-files).  
  - How `create_agent.sh` can set `agent_memory_subdir` via its `memory_subdir` argument: [TBC_LIBRARY_INSTALLATION.md](TBC_LIBRARY_INSTALLATION.md) and `create_agent.md`.
...
- **behaviour.md**: The memory-backed file containing behaviour rules for an agent. Located at `/a0/memory/<agent_memory_subdir>/behaviour.md`, where `agent_memory_subdir` comes from `/a0/tmp/settings.json`. Agents should use the `behaviour_adjustment` tool to edit it. See [TBC_LIBRARY_AGENT_REASONING.md → Memory vantage and backup-safe files](TBC_LIBRARY_AGENT_REASONING.md#memory-vantage-and-backup-safe-files).
...
- **Config-First Rule**: The principle that agents should always read configuration files (`/a0/tmp/settings.json`, `/a0/tmp/system_control.json`) as JSON before inferring configuration from filesystem patterns or broad searches. See [TBC_LIBRARY_AGENT_REASONING.md → Config-First Rule](TBC_LIBRARY_AGENT_REASONING.md#config-first-rule).
- **Common layer**: The shared host directory `layers/common_layer` (seen inside the container as `/common_layer` and via specific `/a0/...` mounts) that holds shared agents, knowledge, prompts, and instruments used by many agents.
...
- **Knowledge tree**: A structured directory of narrative, conceptual, or procedural documents under `layers/common_layer/knowledge/...` on the host, exposed inside the container under `/a0/knowledge/...`. See [TBC_LIBRARY_EXTENSIBILITY.md → Knowledge Features of Agent Zero](TBC_LIBRARY_EXTENSIBILITY.md#knowledge-features-of-agent-zero).
