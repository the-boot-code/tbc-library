...
- **Inside-container library-aware orchestrator**  
  The same in-container agent can reason about other agents and containers within the same `tbc-library` deployment by using `/containers` and `/layers` (bind-mounted from that deployment's host directories) together with `/agent_orchestration` and `/agent_layer` (self-specific views). This provides a clear separation between "my own container" and "other containers/agents in this deployment". To discover another agent's configuration and memory root, apply the **Config-First Rule**: read their `settings.json` and use `agent_memory_subdir` to locate their `behaviour.md`. See [TBC_LIBRARY_AGENT_REASONING.md → Config-First Rule](TBC_LIBRARY_AGENT_REASONING.md#config-first-rule) and [Memory vantage and backup-safe files](TBC_LIBRARY_AGENT_REASONING.md#memory-vantage-and-backup-safe-files).
